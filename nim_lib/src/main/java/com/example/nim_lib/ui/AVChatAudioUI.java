...
import com.example.nim_lib.common.util.NetworkUtil;
import com.example.nim_lib.common.widgets.ToggleListener;
import com.example.nim_lib.common.widgets.ToggleState;
import com.example.nim_lib.common.widgets.ToggleView;
import com.example.nim_lib.constant.AVChatExitCode;
import com.example.nim_lib.constant.CallStateEnum;
import com.example.nim_lib.controll.AVChatController;
import com.example.nim_lib.module.AVChatControllerCallback;
import com.example.nim_lib.module.AVSwitchListener;
import com.example.nim_lib.ui.activity.CallWaitActivity;
...
//    private static final int[] NETWORK_GRADE_DRAWABLE = new int[]{R.drawable.network_grade_0, R.drawable.network_grade_1, R.drawable.network_grade_2, R.drawable.network_grade_3};
    ...
    private void findViews() {
        ...
        switchVideo = rootView.findViewById(R.id.tv_switch);
        switchVideo.setOnClickListener(this);
//
        headImg = rootView.findViewById(R.id.iv_avatar);
        nickNameTV = rootView.findViewById(R.id.tv_name);
...
//        wifiUnavailableNotifyTV = rootView.findViewById(R.id.avchat_audio_wifi_unavailable);
        notifyTV = rootView.findViewById(R.id.tv_content);
...
//        }
    ...
    public void onClick(View v) {
        int i = v.getId();
        if (i == R.id.tv_refuse) {//拒绝
            doRefuseCall();
        } else if (i == R.id.tv_receive) {//接受
            doReceiveCall();
        } else if (i == R.id.tv_hang) {//挂断
            doHangUp();
        } else if (i == R.id.tv_mute) {//静音
            avChatController.toggleMute();
        } else if (i == R.id.tv_hands_free) {//免提
            avChatController.toggleSpeaker();
        } /*else if (i == R.id.avchat_audio_record) {
            doRecording();
        }*/ else if (i == R.id.tv_switch) {//切换摄像头
            if (isInSwitch) {
...
//        });
    ...
    private void closeSession() {
        ((CallWaitActivity) context).finish();
    }
}
