...
public class RequestParamConvert {
    ...
    public static String createLinkString(Map params, boolean isIncludeSign, boolean isEncode, boolean isDecode, boolean isSort) {
        List<String> keys = new ArrayList<String>(params.keySet());
        if (isSort) {
            Collections.sort(keys);
        }
        ...
        for (int i = 0; i < keys.size(); i++) {
            ...
            String value = params.get(key)+"";
            try {
                if (!"sign".equals(key) || isIncludeSign) {
                    if (i == keys.size() - 1) {//拼接时，不包括最后一个&字符
                        if (isEncode) {
                            prestr = prestr + key + "=" + URLEncoder.encode(value, "utf-8");
                        } else if (isDecode) {
                            prestr = prestr + key + "=" + URLDecoder.decode(value, "utf-8");
                        } else {
                            prestr = prestr + key + "=" + value;
                        }
                    } else {
