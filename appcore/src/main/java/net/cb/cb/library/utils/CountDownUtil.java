...
public class CountDownUtil {
     ...
     * @param defaultText 计时结束后view上显示的内容
     */
    public static void getLogoutTimer(int second, final View view, final String defaultText, final Context context, final CallTask callTask) {
        timer = new CountDownTimer(second * 1000 + 1500, 1000) {
            @SuppressLint("DefaultLocale")
            ...
            public void onTick(long millisUntilFinished) {
                ...
                //如果是TextView的话,设置显示倒计时同时设置view不可点击
                if (view instanceof TextView) {
                    ((TextView) view).setText(defaultText + String.format("(%ds)", remainderTime));
                    ((TextView) view).setTextColor(ContextCompat.getColor(context, R.color.gray_250));
                }
                if (remainderTime == 0) {
                    //判断为1秒时,结束计时,并恢复view可以点击
