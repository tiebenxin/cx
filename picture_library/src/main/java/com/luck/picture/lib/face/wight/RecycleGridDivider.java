package com.luck.picture.lib.face.wight;
...
public class RecycleGridDivider extends RecyclerView.ItemDecoration {
    ...
    public RecycleGridDivider(int space, int color, int type) {
        this.space = space;
        this.color = color;
        mPaint = new Paint(Paint.ANTI_ALIAS_FLAG);
        mPaint.setColor(color);
    ...
    public RecycleGridDivider(int space, Drawable mDivider) {
        this.space = space;
    ...
    private void drawGrideview(Canvas canvas, RecyclerView parent) {
        ...
        for (int i = 0; i < childSize; i++) {
            ...
            if (i < other) {
                right = child.getMeasuredWidth() * (i % other + 1) + left + space * i;
            } else {
                right = child.getMeasuredWidth() * (i % other + 1) + space * i;
            }
            if (mDivider != null) {
                mDivider.setBounds(left, top, right, bottom);
                mDivider.draw(canvas);
