...
import com.listener.CheckPermission;
...
public class CaptureButton extends View {
    ...
    public CaptureButton(Context context, AttributeSet attrs) {
        ...
        button_size = typedArray.getDimensionPixelSize(R.styleable.VideoButton_button_size, 100);
        typedArray.recycle();
        init(button_size);
    }
    ...
    private class LongPressRunnable implements Runnable {
        @Override
        public void run() {
            setState(STATE_LONG_PRESS);//如果按下后经过300毫秒则会修改当前状态为长按状态
            //没有录制权限
